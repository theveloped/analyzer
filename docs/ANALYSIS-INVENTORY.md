...
| `roughing`      | `prep/directions` (+ `prep/voxels` internally)         | `voxel` (auto ~4M bbox cells), `tollerance`, `film_voxels` 1, `min_volume` (auto 64 voxels), `direction_indices`, `tools`, `reach_tollerance` 0.1, `wall_tollerance` 1°, `pixel`, `window` 0.3 | `part_volume`/`hull_volume`/`residual_volume` exact, `labeled_volume` + `volume_error` (voxel vs exact), `pockets[]` (volume, area, `max_depth`, `best_tool`, `tool_area_fraction`), `unreachable_volume`, dropped/unassigned counts | `pocket_id` u4/face (0 = on the hull)                                                                                              | ROUGHING_SCHEMA 1                                                                            |
...
| cnc      | `hull`                  | cnc/hull                                                                   |        |                                        |
| cnc      | `roughing`              | cnc/roughing                                                               |        | pocket volumes + tool                  |
