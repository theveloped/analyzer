...
> The v2 workbench (lenses · studies · operations · checks) is described in
> docs/ROUTE-ARCHITECTURE.md — read that before picking up any v2-frontend or
> route-layer work. Items 9, 12 and 17 below intersect with it.
>
> Cut 2026-07-31: decisions/candidacy slots, `$plan` bindings, route templates,
> per-kind default checks, reports and dispositions. Items 22 and 23 were about
> that machinery and are gone with it.
...
### 24. The coverage study: what the route machines, together

**Problem.** After the lean cut there is a study for candidate DIRECTIONS
(rows = candidates, columns = visible/revolvable/swept/per-tool reach,
footer = the union over the selection) but nothing that compares the
OPERATIONS you actually authored. "Is this route complete?" is answered
only by the `reach_route` check's single number.

**What exists to build on.** `machining.reweight_option` already computes
per-setup `reachable` / `exclusive` / `marginal` plus `multi` and
`internal`; `machining.setup_defaults` already computes the per-BREP-face
assignment (earliest covering setup wins, 254 = conflict wanting a user
cut, 255 = unmachinable). Both are driven today only by the automatic
`cnc/setups` search. The frontend half exists as `v2/table/columns.ts`
(`ColumnDef`) + `v2/table/aggregate.ts` (union share) + `reach.opReach`.

**Approach.** (a) Make `ColumnDef` row-generic and turn `fieldId(row)` into
`fieldIds(row): string[]` — an operation's mask is a set, a direction's is
the one-element case; the directions study is unchanged. (b) A shared
`v2/table/coverage.ts` mirroring `reweight_option` + `setup_defaults` over
the reach masks, read by BOTH the study table and a new
`cnc:setup_assignment` lens so the number and the picture cannot disagree.
(c) A `machining_coverage` study whose rows are the route's operations,
with **exclusive** and **marginal** columns — the answer to "does this
operation earn its setup". (d) Conflicts (254) offer the existing
`splits.py` cut.

**Do NOT** make this a new AnalysisDef: an analysis keyed on the operation
list would re-key on every edit, losing the property that operation edits
are interpretation-only.

**Verify.** A vitest over hand-computed masks; cross-check the totals
against `cnc/setup_verdict` areas on `testpart_42`.
...
**Now more urgent.** With per-kind default checks gone (2026-07-31), a lens
band is one of only two ways a check ever reaches the route — a save that
silently produces nothing is no longer a corner case.

**Verify.** Save a band on each of the seven field lenses; each renders a
card. A vitest case asserting every `FIELD_LENSES` entry resolves through
`describeCheck`. Frontend `npx tsc -b`, `npm test`.
