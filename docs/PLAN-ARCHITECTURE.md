...
- **Plan** — work toward a full production plan, DFM-checked along the way: a left rail of
  operations carrying their checks, prefillable from machine templates, loose
...
Defined once, in **[CONCEPTS.md](CONCEPTS.md)** — lens, study, check, policy, finding,
disposition, candidate, decision, plan, operation and the rest, each with its code
anchor and whether anything enforces it. That file is canon for what a word means;
this one is canon for how the plan layer is built.

The two rules this document leans on hardest:

- **Status is four independent axes, never one field** — execution · verdict ·
  disposition · audience. "Computed" is not "good".
- **A lens is never a verdict; a check never computes.**
...
- **What a computation was ASKED about is a declared param; which answer you read is
  not.** A candidate-indexed analysis (`cnc/reach_study`, `cnc/turning_scan`) declares
  the candidates it covers and emits one row per candidate, so changing which candidate
  you are *looking at* is interpretation and costs nothing. Since `resolver.cache_key`
  filters submitted params down to declared ones, a `candidate_id` bolted onto the
  params dict is silently dropped. So such an analysis is keyed by whatever
  candidates it was ASKED about — one, or all of them — and a comparison surface merges
  every non-stale result of that analysis rather than demanding one that covers exactly
  the current set. That is what lets a table fill in a cell at a time instead of
  forcing an all-or-nothing run, and it means candidates cost nothing to define:
  `cnc/turning_scan` takes vectors and needs only the mesh. The exception is genuinely
  set-shaped artifacts — `accessibility.npy` is one array over all directions, so
  computing it necessarily covers the whole set, and the UI should say so rather than
  pretend otherwise.
...
5. **v2 UI**: operation cards in the pipeline rail (direction dropdown from
...
4. **Read-only view**: v2 sidebar gains a Reports list per part; opening one renders the
   ...
   dispositions ("accepted — why"), part/plan revision provenance. The audience axis is
...
| 1. Plan substrate | `api/plan.py`; plan/history/dispositions sidecars; manifest `plan` section; pinned policies; evaluators + findings derivation; PipelineRail carries operations + checks; quotation-input fields on operations |
