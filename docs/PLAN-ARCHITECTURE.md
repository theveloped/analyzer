...
| **Candidate set** | An enumerated, user-curatable list of options for one decision slot (directions today; stock and bend order later). | Generated by a study or by geometry, then edited — never implicit. |
| **Decision** | A slot holding a candidate set, a selection and a lifecycle state (`plan.decisions.<slot>`, `plans.py`). | The selection's projection to `value` is DERIVED (`normalize_decisions`), never authored — it is the stable path checks bind to. |
...
## Keying rules (the three footguns)

- **What a study was ASKED about is a declared param; which answer you read is not.**
  A study over candidates (`cnc/reach_study`, `cnc/turning_scan`) declares the
  candidates it covers and emits one row per candidate, so changing which candidate you
  are *looking at* is interpretation and costs nothing. Since `resolver.cache_key`
  filters submitted params down to declared ones, a `candidate_id` bolted onto the
  params dict is silently dropped. So a study is keyed by whatever
