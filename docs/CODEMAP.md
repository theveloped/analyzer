...
- `tube_laser` — `profile` (TUBE_SCHEMA mirrored in `frontend/src/processes/tubelaser/index.ts`; FEATURES_SCHEMA likewise in `cnc/features.ts`, HULL_SCHEMA in `cnc/hull.ts`, ROUGHING_SCHEMA in `cnc/roughing.ts`)
...
| `v2/decisions/` | the candidate → decision layer: `directions.ts` owns the `directions` slot (generator params → `prep/directions`, `direction_sources` → candidates, selection/suppression writes), `columns.ts` assembles the overview rows from study STATS only — no field fetches — and runs a whole column's study, because the backend runs one job per part |
