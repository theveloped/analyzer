...
| `registry/types.ts` | **the plugin contract**: `ProcessPlugin { processId, modes: ViewMode[], defaults, paramWidgets?, inspect?, onPick? }`; `ViewMode.paint(ctx) → PaintInfo` and `ViewMode.params?: ViewParamSpec[]` (the knobs that mode's paint reads — the rail generates its form from these); `ViewCtx` gives `getField`, `paintFaces`, `setLines/Arrows/Graph`, params. `ProcessPlugin.Controls?` is the retired per-process panel: only `directions` still supplies one and only the v1 `App.tsx` renders it |
| `registry/index.ts` | plugin list — register new process plugins here |
| `processes/cnc/` | CNC plugin: verdict/gap/stickout modes (`modes.ts`), client-side tool composition (`compose.ts`), field lookup (`sources.ts`), per-mode param declarations + the manifest-derived source/tip widgets (`params.tsx`); setup-assignment view has the "optimize parting lines" button |
...
| `v2/components/rail/` | the rail vocabulary every panel is built from: `Rail` (the body + its rhythm), `RailHeader` (identity: icon · title · status · blurb · actions/close), `RailSection`/`RailDivider`, `RailAlert`/`RailError`/`RailStats`, `RailDisclosure`, `RailRunButton`, the field widgets and `ParamsForm`, plus `RailEditorShell` — the sibling shell for a surface that AUTHORS rather than inspects (PMI), where the save is the point and so the actions region is explicit and dirty-driven instead of a trailing affordance. The header comment states the slot order — identity, alert, params, advanced, action, findings, readout — of which only params → action → findings is ordered by meaning ("thresholds → run → findings"); advanced sits ABOVE the action because compute knobs are inputs to the run |
| `v2/workspace/modeRails.ts` | `MODE_RAILS` / `LENS_ACTIONS`: the modes whose rail cannot be declared as params (a viewer pick, an authoring surface, a job-param mapping) name their own component here rather than the dispatch growing a branch |
