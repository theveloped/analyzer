...
| `turning.py` | Lathe axis (Plücker least-squares on the surface normals), the maximal turned state (smallest solid of revolution containing the part) and per-face turning roles | `analyse_turning`, `seed_axes`, `refine_axis`, `outer_profile`, `classify_faces` |
...
- `cnc` — `features`, `turning`, `setups`, `setup_verdict`, `reach_study`, `hull`, `precompute`, `compose`. `turning` (TURNING_SCHEMA mirrored in `frontend/src/processes/cnc/turning.ts`) requires only `prep/mesh` — it reads `brep_meta.json`/`brep_faces.npy`/`brep_edge_pairs.npy`/`normals.npy`, all mesh-time artifacts, and deliberately does *not* declare `prep/aag` so it still runs on an STL part (where `compute_aag` would raise); it opts into the `splits` salt because roles are voted per effective face. npz: `turn_role` u1 (category over `turning.TURN_ROLES`), `turn_residual` f4 (azimuthal tilt in degrees, per FACE — so the viewer paints it directly rather than through the per-vertex `heatmapMode`), `milled_region` u4. `stats.profile` is the maximal turned state's meridian in **axial** coordinates: `[z, r]` pairs measured from `stats.axis.point` along `stats.axis.direction`, so a world point is `point + z*direction + r*u` for any `u` perpendicular to the axis
