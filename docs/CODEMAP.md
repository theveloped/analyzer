...
| `zmap.py` | 2D height-map (Z-map) engine: renders depth maps, grayscale closings, Euclidean gaps, clearance fields; owns the per-direction cache | `render_heightmap`, `face_visibility`, `point_visibility`, `close_heightmap`, `slenderness_ladder`, `euclidean_gap`, `clearance_heightmap`, `tip_aware_min_stickout`, `DirectionCache`, `compose_unreachable` |
...
| `brep.py` | BREP-aware STEP meshing via OCCT (OCP bindings): per-face tessellation, welding, conformal subdivision, exact analytic surface normals | `mesh_step`, `subdivide_tagged`, `surface_normals_at`, `analytic_face_normals` |
...
| `turning.py` | Lathe axis (Plücker least-squares on the surface normals), the maximal turned state (smallest solid of revolution containing the part) and per-face turning roles | `analyse_turning`, `seed_axes`, `refine_axis`, `outer_profile`, `inner_profile`, `boundary_membership`, `face_metrics`, `classify_effective_faces`, `split_state` |
...
| — | — | A result field's `association` says which index space it lives in: `face`/`vertex` are the FINE mesh, `brep_face` is the BREP (valid against the coarse preview too — the viewer joins it through `loadBrepFaceIds`), `none` is unindexed geometry. Before the fine mesh exists the manifest publishes **only** `brep_face` fields; painting a fine-indexed array onto the coarse preview would be silently wrong, so it is made impossible rather than discouraged |
...
| `directions_sources.json` | directions | per-row provenance (`source`, `label`, `detail`), index-aligned to `directions.npy`, plus the area-weighted `accessible_area` / `accessible_fraction` for that row — so a per-direction overview reads shares without fetching a single accessibility mask |
...
- `prep` — `mesh_coarse`, `mesh`, `aag`, `directions`, `voxels`, `attributes`,
  ...
  is the shared SDF (moved here from `injection_molding/flow_voxels`, which is
  gone; the injection plugin and `flow_fill` both run `prep/voxels`, and
  `voxels.ts` still reads results left in the old location)
- `cnc` — `features`, `turning`, `turning_scan`, `setups`, `setup_verdict`, `reach_study`, `corner_access`, `hull`, `precompute`, `compose`. `corner_access` (CORNER_SCHEMA, mirrored in `frontend/src/processes/cnc/corners.ts`) is the EDGE-level sibling of `reach_study`: a cutter is round, so a sharp internal corner cannot be reproduced by one however well the tool reaches the faces either side. It requires `prep/directions` + `prep/aag` and is the first consumer of the AAG's per-edge `polyline_points`/`polyline_offsets`; the only geometry it pays for is one `zmap.render_heightmap` per direction, everything else being numpy over `aag.npz` + `brep_meta.json` + `normals.npy` (no STEP reload — `brep.surface_normals_at` evaluates the five analytic quadrics at the edge samples, since a face's mid-UV normal is useless at its boundary). Per sampled segment: occluded, or either adjacent face back-facing → `blocked`; tangent perpendicular to d with one side a floor → `sharp` (a flat endmill's bottom edge IS a sharp circle, so this holds however the wall is sloped, as long as it does not overhang); tangent parallel to d → `radius` (exactly D/2); anything else visible → `oblique` (≥ D/2, not modelled exactly). `edge_class` picks the convexity: `concave` is the PART's internal corners (the CNC question), `convex` is a mold CAVITY's, which is what the `injection_molding:moldCorners` lens runs. Arrays store the segment geometry ONCE (`segment_points` f4 (S,2,3) role `lines`, `segment_edge` u4) and the verdicts per direction (`segment_role_<d>` u1, `edge_role_<d>`/`edge_radius_<d>` over canonical BREP edge ids, `face_flag_<d>` u1 association `brep_face`); `EDGE_ACCESS_ROLES` travels inside `field_meta["types"]` the way `machining_features.FEATURE_TYPES` does, so the labels are read off the descriptor rather than mirrored as a TS union. Deliberately crude and NOT a substitute for `reach_study` / `compose`, which stay the authority on whether a tool actually fits: no flank, holder or stickout model. `turning_scan` (TURNING_SCAN_SCHEMA) is the cheap sibling of `turning`: `axis_vectors` in — VECTORS, not indices, so it requires only `prep/mesh` and asking whether an axis is turnable never waits on an accessibility run — and one `stats.axes[]` row per candidate out, each carrying the direction as given so a caller joins its own candidate list back on without any shared index space. Schema 2 stores `axis_role_<k>` u1 per fine face for each scanned axis (0 off-axis, 1 revolution-compatible, 2 swept) — `turning.axis_roles` produces the array and the fractions in one call, so a painted area share IS the reported number rather than an estimate of it, and unioning roles across axes is how the directions overview aggregates a selection. That is why the scan runs at FULL resolution and no longer takes `sample_faces`: once a per-face field has to be stored the pass is paid for. One axis per call is a perfectly good call — distinct `axis_vectors` are distinct cache entries and the overview merges them, which is what lets a table fill one cell at a time. Painted by the `cnc:axis_role` lens, pinned with `scanHash` + `scanAxis` (not "the latest result", which per-cell runs make meaningless). Per axis it is `turning.azimuthal_residual` + `turning.score_axis` — pure O(F) numpy over (centroid, normal, area, axis) with the axis-independent `c x n` / `|c|^2` hoisted — measured at ~8 ms per axis on a 100k subsample against ~7 s for a full `turning` run with `axis_override`, which is why the overview scans instead of looping. It reports TWO fractions and neither is `turning`'s `turned_area_fraction`: `inlier_fraction` (revolution-compatible area) reads ~0.9 on a drilled plate because every plane perpendicular to any axis is trivially a surface of revolution about it, so `radial_fraction` (actually swept) is the number that separates a lathe part from a plate — `qualified` applies the same `MIN_RADIAL_FRACTION` gate the full analysis uses. `_best_line_along` solves the perpendicular offset by area-weighted 2x2 least squares first, because a candidate is a direction and not a line, and scoring the line through the part centroid would under-report every off-centre axis. `turning` (TURNING_SCHEMA mirrored in `frontend/src/processes/cnc/turning.tsx`) requires only `prep/mesh` — it reads `brep_meta.json`/`brep_faces.npy`/`brep_edge_pairs.npy`/`normals.npy`, all mesh-time artifacts, and deliberately does *not* declare `prep/aag` so it still runs on an STL part (where `compute_aag` would raise); it opts into the `splits` salt because roles are voted per effective face. npz: `turn_role` u1 (category over `turning.TURN_ROLES` — `other`, `od_face`, `od_turn`, `id_face`, `id_turn`, `on_axis`; per fine triangle, and on a face flagged as needing a split it is the triangle's OWN role rather than the face's), `turn_residual` f4 (azimuthal tilt in degrees, per FACE — so the viewer paints it directly rather than through the per-vertex `heatmapMode`), `milled_region` u4, plus `brep_valid` u4 / `brep_default` u1 indexed by EFFECTIVE face id (association `none`, same contract as `cnc/setups`): bit `r` of `brep_valid` is set iff role `r` covers the whole face, and `brep_default` is that role or `turning.CONFLICT_ROLE` (254) when the face is part turned and part not and wants a user cut. `profile_bins` genuinely sets the profile resolution: each bin's radius is the EXACT per-bin maximum, because `r(z)` is convex along a straight edge and so peaks only at vertices or at bin-boundary crossings, both of which are sampled. `stats.profile` is the maximal turned state's meridian in **axial** coordinates: `[z, r]` pairs measured from `stats.axis.point` along `stats.axis.direction`, so a world point is `point + z*direction + r*u` for any `u` perpendicular to the axis. `stats.inner_profiles` is the matching contour of the bores in the same coordinates: a LIST of polylines, separate from the outer one because a bored part's meridian is a region with holes, and one entry per contiguous bored run because a part bored from both ends has solid material between (joining those would draw a line straight through it)
- `injection_molding` — `mold_orientation`, `thickness`, `gaps`, `ray_thickness`, `ray_gap`, `slenderness`, `thin_span`, `wall_skeleton`, `sprue_proposals`, `ejection_sticking`, `flow_fill`
- `sheet_metal` — `detect`, `flat_pattern` (SHEET_SCHEMA mirrored in `frontend/src/processes/sheetmetal/index.ts`), `bend_plan` (BENDPLAN_SCHEMA mirrored in `sheetmetal/bendplan.ts`). All three require only `prep/mesh_coarse` + `prep/aag`: their role/panel fields are `brep_face`, so they run and paint on the first-load preview. `bend_plan` is the one exception in kind — its fold mesh is genuinely per fine VERTEX, so it is skipped (with a reason in `stats.fold_mesh`) when no fine mesh exists and the analysis declares `salts=("mesh",)` so the coarse-only result and the full one are separate cache entries rather than one masquerading as the other. Schema-2 bend_plan additions: npz `flat_verts` f4 (3V, pattern-frame fold coordinates, z = material height with mid-surface at z_offset), `vertex_panel`/`vertex_bend` u1 (+1-encoded owners), `bend_t` f4, optional `collision_faces` u1 (mesh_check hits); stats gain `fold_mesh` (availability + base_transform), `tooling` (referenced punch/die/machine YZ profiles), per-plan-step `placement`/`lift_sign`/`theta_before`/`phi_target` (machine pose for the bend-sequence animation), and `mesh_check`. Viewer scene capabilities backing the animation: `Scene3D.setVertexPositions` / `addOverlayMesh`+`shiftOverlay` / `setAnimator` (reset on every repaint)
...
POST /api/parts                      upload STEP/STL → content-addressed workdir parts/<sha1[:12]> (idempotent); returns {part, job} — the job is the first-load bundle, and the client MUST watch it or the coarse preview never appears without a reload
...
| `v2/decisions/` | the candidate → decision layer: `directions.ts` owns the `directions` slot (`direction_sources` → candidates, selection/suppression writes), `columns.ts` assembles the overview rows from study STATS only — no field fetches — and owns the COLUMN TABLE (`SCALAR_COLUMNS` + `toolColumn`): one `ColumnDef` carries a column's label, its cell, the job that fills it, the field its value came from and how that field encodes "covered", so the header, the runner, the tooltip and the aggregate footer all read one entry instead of eight `===` dispatches that had to agree |
| `v2/table/` | `SortableHeader.tsx` (ported verbatim from wf-api; the vendored Catalyst table is byte-identical) + a client-side `useTableControls` — same filter/sort contract as the Partner Portal, minus the server query string and pagination |
| `v2/workspace/RightRail.tsx` | the right column's only chrome: border, scroll box and the drag handle, with a runtime width remembered per rail id (Tailwind cannot emit an arbitrary width, which is why the rails themselves are just content) |
| `v2/studies.ts` | the third surface beside lenses and checks: a study lays many candidates out side by side. Opening one activates its lens with `shownKeys: []` so the viewer starts on the bare model and the table's selection adds arrows back |
| `v2/decisions/aggregate.ts` | the study footer: area covered by AT LEAST ONE selected direction, unioned over the masks (per-pair stats overlap, so they cannot answer it). Clicking a total paints the same union through the `cnc:coverage` lens, which takes the field ids and the coverage rule as viewer params rather than needing a mode per column; both sides share the `covered()` predicate in `processes/cnc/coverage.ts`, so the total and the picture cannot disagree |
