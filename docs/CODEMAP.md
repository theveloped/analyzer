...
| `turning.py` | Lathe axis (Plücker least-squares on the surface normals), the maximal turned state (smallest solid of revolution containing the part) and per-face turning roles | `analyse_turning`, `seed_axes`, `refine_axis`, `outer_profile`, `inner_profile`, `face_metrics`, `classify_effective_faces`, `split_state` |
