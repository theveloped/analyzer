...
def get_distance(mesh_a, mesh_b, upper_limit: float = 3.4028234663852886e+38, lower_limit: float = 0.0):
    res = mm.projectAllMeshVertices(mesh_a, mesh_b, upDistLimitSq=upper_limit, loDistLimitSq=lower_limit)
    return res.vec
...
def map_result_faces(mesh_a, mesh_b, faces_a, min_range=None, max_range=None, n_intersect = 3):
    distances = get_distance(mesh_b, mesh_a, upper_limit=10, lower_limit=-10.0)
    ...
    inside_faces = [i for i, face in enumerate(faces_a) if len(set(face).intersection(indices_set)) >= n_intersect]
    return np.array(inside_faces)

@log_execution_time
def map_inter_faces(mesh_a, mesh_b, faces_a, min_range=None, max_range=None):
...
@log_execution_time
def sample_unity_vector_pairs_z(n):
    ...
    indices = np.arange(0, n, 1)
    phi = np.pi * (3. - np.sqrt(5.))  # Golden angle in radians
    z = 1 - (indices / ((n) - 1)) * 2  # y goes from 1 to -1, adjusted for n points
...
@log_execution_time
def sample_unity_vector_datums(n):
    # create simple dirs array
